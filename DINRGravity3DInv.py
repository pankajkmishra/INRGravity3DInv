...
# Date: July 31, 2025
#
# This script performs a 3D gravity inversion using a Deep Implicit Neural Representation (DINR).
# The goal is to recover a subsurface density model from surface gravity measurements.
#
# - What is a Deep Implicit Neural Representation (DINR)?
#   -- A DINR is a method where a neural network is trained to represent a complex signal or field
#      as a continuous function. Instead of storing discrete values in a grid (like pixels in an image
#      or voxels in a 3D model), the network learns a mapping from input coordinates (e.g., x, y, z)
#      to an output value (e.g., density).
#   -- The network's weights *implicitly* encode the entire 3D density field. This approach is
#      memory-efficient, as the model size is independent of the desired output resolution, and
#      it naturally provides a continuous representation that can be queried at any point in space.
#
# - How this script works:
#   - Setup and Synthetic Data Generation:
#     -- The script starts by setting up the environment, including defining the physical grid dimensions
#        and observation locations.
#     -- A "true" 3D density model is created using a Gaussian Random Field (GRF) to simulate a
#        geologically plausible, complex subsurface structure.
#   - Forward Modeling:
#     -- A sensitivity matrix 'G' is pre-computed. This matrix mathematically links the density of each
#        subsurface cell to the vertical gravity component ('gz') at each surface observation point, based
#        on the analytical solution for a rectangular prism.
#     -- The "true" gravity data is calculated by multiplying the sensitivity matrix 'G' with the true
#        density model (d = Gm).
#     -- Gaussian noise is added to this data to create the final "observed" gravity data, simulating
#        real-world measurement inaccuracies.
#   - Inversion with a Neural Network:
#     -- A neural network ('DensityModel') is defined. This network takes 3D spatial coordinates as input
#        and outputs the corresponding density value. This is the core of the DINR.
#     -- The network is trained to minimize the difference between the gravity data predicted from its
#        density model and the "observed" gravity data.
#     -- The training loop works as follows:
#        --- The network predicts a density value for every point in the 3D grid.
#        --- This predicted density model is used to calculate the predicted gravity data.
#        --- The Mean Squared Error (MSE) between the predicted and observed gravity data is computed.
#        --- This error (loss) is backpropagated through the network to update its weights, gradually
#            improving the density model.
#   - Evaluation and Visualization:
#     -- After training, the final inverted density model is generated by the trained network.
#     -- The script produces several plots to assess the quality of the inversion:
#        --- A comparison of the true and inverted density models (as 2D slices).
#        --- A comparison of the observed and predicted gravity data.
#        --- The data misfit (residuals) to highlight areas of discrepancy.
#        --- The training loss curve over epochs.
